...
class SetUpUI extends React.Component {
    ...
    toggleShowOneDItems = () => {
        this.setState(state => {
            state.showOneDItems ?
                state.showOneDItems = false :
                state.showOneDItems = true;
            return state;
        });
    }
    toggleShowDataBarItems = () => {
        this.setState(state => {
            state.showDataBarItems ?
                state.showDataBarItems = false :
                state.showDataBarItems = true;
            return state;
        });
    }
    toggleShowPostalCodeItems = () => {
        this.setState(state => {
            state.showPostalCodeItems ?
                state.showPostalCodeItems = false :
                state.showPostalCodeItems = true;
            return state;
        });
    }
    toggleShowFurtherModesItems = () => {
        this.setState(state => {
            state.showFurtherModesItems ?
                state.showFurtherModesItems = false :
                state.showFurtherModesItems = true;
            return state;
        });
    }
                ...
                <Modal.Body className={"overFlowX noPadding"}>
                    <div id="div-settingsModule" className="div-settingsModule">
                        ...
                        <div className="div-advanceSettings div-runtimesettings-details-container" style={this.state.showFurtherModesItems ? style.show : style.hide}>
                            <Card style={{
                                padding: '1vw', marginTop: '1vw', display: 'none'
                                /** JS not supported yet */
                            }} id="div-runtimesettings-accompanyingTextRecognitionModes-container" className=" div-runtimesettings-mode-container">
