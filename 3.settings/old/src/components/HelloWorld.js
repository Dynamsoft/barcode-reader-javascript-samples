...
import { Alert, Tabs, Tab, Badge, Button, Modal, Spinner, Card } from 'react-bootstrap';
...
class HelloWorld extends React.Component {
    ...
    render() {
        return (
            <>
                <h1><Badge variant="secondary">{this.props.title}</Badge></h1>
                <Tabs defaultActiveKey="reader" id="uncontrolled-tab-example">
                    <Tab eventKey="home" title="Home">

                    </Tab>
                    <Tab eventKey="reader" title="Read Existing Files">
                        <h3><Alert variant="info">Read Barcodes from existing files(PNG, BMP, JPG, GIF and PDF)</Alert></h3>
                        <input className="btn" onChange={this.onIptChange} type="file" multiple accept="image/png,image/jpeg,image/bmp,image/gif" />
                        <Button className="btn" variant="info" onClick={this.showRuntimeSettings}>Show Runtime Settings</Button>
                        <Button className="btn" variant="info" onClick={this.setUpRuntimeSettings}>Check or Update Runtime Settings</Button>
                        {(() => {
                    ...
                    <Tab eventKey="scanner" title="Read From Video Stream" disabled>
                    ...
                    this.state.bInitializing ? (<div style={{ textAlign: 'center', padding: '2vw' }}><Spinner animation="border" /></div>) :
                        (<div id="div-settingsModule" className="div-settingsModule">
                            <div className="div-advanceSettingsHeader">
                                <a title="Check Documentation" href="https://www.dynamsoft.com/help/Barcode-Reader/struct_dynamsoft_1_1_barcode_1_1_public_runtime_settings.html" target="_blank" rel="noopener noreferrer" original-title="document">
                            ...
                            <div style={{ textAlign: "center", paddingTop: "1vw" }}>
                                <Button variant="outline-info" onClick={this.showCurrentSettings}>Show Current Settings</Button>
                                <Button variant="outline-success" onClick={this.updateRuntimeSettings}>Update Runtime Settings</Button><br /><br />
        ...
        (async () => {
            try {
                ...
                for (let i = 0; i < files.length; ++i) {
                    ...
                    this.appendMessage("Reading done in " + timeElapsed.toString() + " milliseconds");
                    if (results.length === 0) {
                        this.appendMessage("No barcode found in " + timeElapsed.toString() + " milliseconds");//, "Caution!");
                    }
                    else {
                        for (let i = 0; i < results.length; i++) {
                            ...
                            for (let j = 0; j < extendedResults.length; j++) {
                                ...
                                /*
                                * not supported in js
                                 if (extendedResults[j].samplingImage.bytes !== null && extendedResults[j].samplingImage.bytes.length !== 0)
                                     this.appendDetails("Sampling Image (base64): " + btoa(extendedResults[j].samplingImage.bytes));
                                     */
