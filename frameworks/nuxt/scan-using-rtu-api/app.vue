...
import { ref } from "vue";
import type { BarcodeScanner as BarcodeScannerInstance, BarcodeScannerConfig } from "dynamsoft-barcode-reader-bundle";

const barcodeScannerViewRef = ref<HTMLElement>();
let barcodeScanner: BarcodeScannerInstance | undefined;

onMounted(async () => {
  const { BarcodeScanner } = await import("dynamsoft-barcode-reader-bundle");
  await import("~/dynamsoft.config");
